...
# =====================================================================
# PHOTOMETRIC MASS PARAMETER DERIVATION
#
# Determines all 6 mass parameters from photometric data + topology.
# Completely independent of the Bayesian velocity fit.
# No optimization loop. Runs in milliseconds.
#
# Input: 3.6um photometric decomposition
#   - Scale lengths (ab, Rd, Rg) from surface brightness fit
#   - Approximate masses (Mb, Md, Mg) from M/L = 0.5 assumption
#   - Bulge-to-disk luminosity ratio (Lb/Ld) -> mass ratio at 3.6um
#
# Steps:
#   1. solve_field_geometry on photometric masses -> R_env, R_t, cycle
#   2. Horizon + throat + fd-fg system -> topology-corrected M_total, M_gas
#   3. Lb/Ld ratio splits M_stellar -> M_bulge, M_disk
# =====================================================================
...
def derive_mass_parameters_from_photometry(photometry, a0_eff):
    ...
    This method is completely independent of the Bayesian velocity fit.
    It uses GFD topology (horizon/throat conditions) to correct the
    photometric M/L = 0.5 assumption, then the photometric Lb/Ld ratio
    to split the stellar mass.

    The result is the physically correct mass model. When plugged into
    the GFD base equation, the residual against observations reveals
    the vortex power output (sigma).

    Parameters
    ----------
    photometry : dict
        Photometric decomposition from 3.6um imaging:
            'Mb' : float - approximate bulge mass (M/L=0.5, solar masses)
            'ab' : float - bulge scale length (kpc)
            'Md' : float - approximate disk mass (M/L=0.5, solar masses)
            'Rd' : float - disk scale length (kpc)
            'Mg' : float - gas mass from 21cm (solar masses)
            'Rg' : float - gas scale length (kpc)
        The luminosity ratio Lb/Ld is inferred from Mb/Md (constant
        M/L at 3.6um means luminosity ratio = mass ratio).
    a0_eff : float
    ...
    dict with keys:
        'mass_model' : dict
            {'bulge': {'M', 'a'}, 'disk': {'M', 'Rd'}, 'gas': {'M', 'Rd'}}
        'M_total' : float
            Topology-corrected total baryonic mass.
        'field_geometry' : dict
            R_env, R_t, cycle from solve_field_geometry.
        'topology' : dict
            Intermediate values: M_horizon, M_throat, cycle.
    """
    ...
    # ---- Step 1: Field geometry from photometric masses ----
    ...
    # Horizon condition: M_enc(R_env) = yN_horizon * R_env^2 * a0 / G
    ...
    # Enclosed fractions at R_env and R_t using photometric scale lengths
    ...
    # Iterative 3x3 solve: horizon, throat, and fd-fg relationship
    #   Row 1: fb(R_env)*Mb + fd(R_env)*Md + fg(R_env)*Mg = M_horizon
    #   Row 2: fb(R_t)*Mb   + fd(R_t)*Md   + fg(R_t)*Mg   = M_throat
    #   Row 3: Md - ratio*Mg = 0  (from empirical fd = -0.826*fg + 0.863)
    ratio = 2.0
    sol = None
    for _ in range(30):
    ...
    # ---- Step 3: Photometric Lb/Ld split ----
    # At 3.6um, M/L is approximately constant for old stellar populations.
    # The luminosity ratio Lb/Ld maps directly to the mass ratio Mb/Md.
    # This ratio cancels any absolute M/L uncertainty.
